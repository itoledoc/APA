...
class ObsProject(object):
    ...
    def __init__(self, xml_file, path='./'):
        ...
        self.code = self.data.code.pyval
        self.prj_version = self.data.version.pyval
        self.staff_note = self.data.staffProjectNote.pyval
        self.is_calibration = self.data.isCalibration.pyval
        self.obsproject_uid = self.data.ObsProjectEntity.attrib['entityId']

        try:
            self.is_ddt = self.data.isDDT.pyval
        except AttributeError:
            self.is_ddt = False

        self.obsprogram = self.data.ObsProgram
        self.list_sciencegoals = self.obsprogram.findall(prj + 'ScienceGoal')

    def get_sg_schedblocks(self):

        sg_sbs = []
        oussg_list = self.obsprogram.ObsPlan.findall(prj + 'ObsUnitSet')
            ...
            for groupous in groupous_list:
                ...
                for mous in mous_list:
                    ...
                    if array == 'ACA':
                        array = 'SEVEN-M'
                    sg_sbs.append([
                        sb_uid, obsproject_uid, ous_id, ous_name, gous_id,
                        gous_name, mous_id, mous_name, array, execount, '']),
        return sg_sbs
