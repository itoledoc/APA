...
HISTORY:
    ...
    2012.12.27:
        - adding a filter if no configuration is found to multiply by a fudge
        factor


    2013.03.04:
        - Adding geometric average for the spatial resolution  and test on twice
...
class arrayRes:
    def __init__(self, arguments):
        ...
        self.array = {0: "C34-1", 1: "C34-2", 2: "C34-3", 3: "C34-4",
                      4: "C34-5", 5: "C34-6", 6: "C34-7"}
        self.read_cycle2()
    ...
    def find_array(self):
        ...
        for arr in self.array:
            ...
            if self.silent:
                print(self.LAS[arr] * scalingFrequency, self.res[arr] *
                      scalingFrequency)


            if (self.LAS[arr] * scalingFrequency >= self.lasObs and
                    self.res[arr] * scalingFrequency <= self.resObs):
    ...
    def find_array2(self, verbose=False):
        """
        Find the array with the obs. input using the precise resolution,
        LAS...
        """
        ...
        for arr in self.array:
            ...
            resArr = math.sqrt(
                self.data[arr][1][index] * self.data[arr][2][index])
            ...
            elevation_factor = abs(1. / math.sin(
                DEG2RAD * (90. - LATITUDE_ALMA + self.declination)))
            ...
            if self.silent:
                if verbose:
                    print("# Array: %s, LAS: %5.2f, RES: %5.2f" % (
                        self.array[arr], lasFreqArr, spatialResolutionArr))
                    print "#          THB: LAS: %5.2f, RES: %5.2f" % (
                        las_estimated, res_estimated)
                    # print("#")

            if self.pointSource:
                if (lasFreqArr >= self.lasObs and
                        self.resObs >= spatialResolutionArr):
                    arrayMatch.append(self.array[arr])
                else:
                    arrayMatch.append("")

            else:
                if lasFreqArr >= self.lasObs and self.resObs >= \
                        spatialResolutionArr >= self.resObs / 2.:
                    arrayMatch.append(self.array[arr])
                else:
                    arrayMatch.append("")
        ...
        if self.acaUse == 'Y':
            ...
            for arr in self.array:
                ...
                if self.pointSource:
                    ...
                    if (self.resObs >= spatialResolutionArr >= self.resObs / 2.
                            and arr < 4):
                        arrayMatch.append(self.array[arr])
                    else:
                        arrayMatch.append("")
    ...
    def find_array3(self, verbose=False):
        """
        Find the array with the obs. input using the precise resolution,
        LAS.... It takes into account a multi-configuration
        """
            ...
            if self.pointSource:
                ...
                if self.resObs >= spatialResolutionArr >= self.resObs / 2.:
                    ...
                    if lasFreqArr <= self.lasObs and arr > 2:
                        arrayMatchLAS[matchArrayCycle2[arr]] = self.array[
                            matchArrayCycle2[arr]]

                        if lasFreqArrAll[
                            matchArrayCycle2[arr]] <= self.lasObs and \
                                matchArrayCycle2[arr] > 0:
    ...
    def run(self):
        ...
        strOut += "### Using CASA simulation with natural weighting " \
                  "(slightly different of THB)"
        ...
        while (notFound and nTry < maxTry and notFoundLAS and
                self.acaUse == 'N'):
            nTry += 1
        ...
        if nTry > 1:
            if self.silent:
                print("# No array configuration found, fudge factor applied "
                      "(Tol = %3.0f %%)" % ((fudgeFactor - 1.) * 100.))

        if notFound and nTry > 1:
            if self.silent:
                print("# No array configuration found, even with fudge factor, "
                      "problem ...")
